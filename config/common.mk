...
ifeq ($(filter RELEASE SNAPSHOT EXPERIMENTAL WEEKLY FINAL BLOODY,$(LIQUID_BUILDTYPE)),)
    LIQUID_BUILDTYPE :=
endif
...
ifeq ($(LIQUID_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        LIQUID_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(LIQUID_EPOCH)-OFFICIAL-$(LIQUID_BUILD)
    else
    ...
    ifeq ($(LIQUID_VERSION_MAINTENANCE),0)
        LIQUID_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(LIQUID_EPOCH)-$(LIQUID_BUILDTYPE)$(LIQUID_EXTRAVERSION)-$(LIQUID_BUILD)
    else
        LIQUID_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(LIQUID_VERSION_MAINTENANCE)-$(LIQUID_EPOCH)-$(LIQUID_BUILDTYPE)$(LIQUID_EXTRAVERSION)-$(LIQUID_BUILD)
    endif
endif

TARGET_PRODUCT_SHORT := $(subst liquid_,,$(LIQUID_BUILDTYPE))

ROM_FINGERPRINT := LiquidRemix/$(PLATFORM_VERSION)/$(TARGET_PRODUCT_SHORT)/$(LIQUID_EPOCH)

ROM_OTAVERSION := LiquidRemix-$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)-$(LIQUID_BUILDTYPE)

PRODUCT_PROPERTY_OVERRIDES += \
    ro.liquid.version=$(LIQUID_VERSION) \
    ro.liquid.releasetype=$(LIQUID_BUILDTYPE) \
    ro.liquid.build.version=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE) \
    ro.modversion=$(LIQUID_VERSION) \
    ro.liquid.fingerprint=$(ROM_FINGERPRINT) \
    ro.liquid.otaversion=$(ROM_OTAVERSION) \
	ro.liquid.build.date=$(LIQUID_EPOCH)
